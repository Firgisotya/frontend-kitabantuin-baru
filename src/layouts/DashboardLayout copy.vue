<template>
	<v-app id="inspire">
		<v-navigation-drawer v-model="drawer" app bottom color="#1f1d2c">
			...
			<div class="text-subtitle-2 text-center">KIWKIW <a href="https://github.com/Abdurozzaq">Abdurozzaq Nurul
					Hadi</a></div>
		...
		<v-app-bar app flat color="#1f1d2c">
			<v-container class="py-0 ml-0 fill-height">

				<v-app-bar-nav-icon class="mr-2 x-small" v-if="$vuetify.breakpoint.mdAndDown" @click="drawer = !drawer">
				</v-app-bar-nav-icon>
		...
		<v-dialog v-model="cari" width="500">
			<v-card>
				...
				<v-card-text>
					<v-text-field width="$vuetify.breakpoint.smAndDown ? 64px : 300px" dense flat hide-details rounded
						solo-inverted label="Enter you keyword" append-icon="mdi-magnify"></v-text-field>
				</v-card-text>
